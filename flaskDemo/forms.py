...
emp=Employee.query.with_entities(Employee.ssn).distinct()#Creating drop down for Employee
results1=list()
for row in emp:
    rowDict=row._asdict()
    results1.append(rowDict)
empChoice=[(row['ssn'],row['ssn']) for row in results1]
...
project=[(row['pnumber'],row['pnumber']) for row in results2]
...
class AssignForm(FlaskForm):
    ssn=SelectField("Employee", choices=empChoice)
    pno=SelectField("Project", choices=project, coerce=int)
    submit=SubmitField("Assign")

    def validate_ssn(self, ssn):
        pairs=Works_On.query.filter_by(pno=self.pno.data)
        if pairs:
            for pair in pairs:
                if str(pair.essn)==str(self.ssn.data):
                    raise ValidationError('That employee has already been assigned to that project.')
